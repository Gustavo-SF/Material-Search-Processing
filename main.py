...
# Clean text
dataframe["description_clean"] = dataframe["MaterialDescription"].apply(lambda x: clean_text(x) if x is not None else "")
dataframe.loc[dataframe.description_clean.str.contains("^\s*$"), "description_clean"] = "undefined"
...
for i in range(len(FINAL_OUTPUT_MP)):
    ...
    k_neighbors[i] = np.delete(np.argsort(FINAL_OUTPUT_MP[i])[0:11],np.where(np.argsort(FINAL_OUTPUT_MP[i])[0:11]==i))
    if end:
...
for i in range(len(k_neighbors)):
    k_neighbors[i][:len(np.delete(k_neighbors[i],np.where(k_neighbors[i]==-1)))] = np.asarray(data_unique.iloc[np.delete(k_neighbors[i],np.where(k_neighbors[i]==-1)),0])

data_unique = pd.concat([data_unique.reset_index(drop=True), pd.DataFrame(k_neighbors)], axis=1)
dataframe = dataframe.merge(data_unique.iloc[:,2:], how='inner', on='description_clean')
